...
function buildPrompt(issue: AnalyzableIssue, codeRoot?: string): string {
  ...
  return [
    ...
    "",
    "【代码提交规范】代码改完后，必须按以下顺序提交（使用 Gerrit）：",
    "1. 先确保本地 Git 状态干净（git status 检查）。",
    "2. 拉取远端最新代码，保持与远端同步（git pull --rebase 或先 fetch 再 rebase）。",
    "3. 完成代码修改。",
    "4. 提交代码，命令依次为：",
    "   - git add .",
    "   - git commit （提交信息按规范需带上对应的 issue 编号，例如：Issue: #1212）",
