...
function buildPrompt(issue: AnalyzableIssue, codeRoot?: string): string {
  ...
  return [
    ...
    "",
    "【代码提交规范——必须由你亲自执行，不是写出来给别人执行】",
    "你已经拥有 Bash 工具，权限已设为自动放行（bypassPermissions），并且本地已配置好 SSH/Gerrit 凭证，git push 会成功。",
    "因此代码改完后，你必须**亲自用 Bash 工具按顺序执行**下列命令，把改动真正提交并推送到 Gerrit，而不是把这些命令输出给用户去手动执行：",
    "  1. git status   （确认改动符合预期）",
    "  2. git fetch origin && git rebase origin/main   （与远端同步；若 rebase 冲突则停下并如实报告，不要强行覆盖）",
    "  3. git add <你改动过的文件>   （只 add 本次实际改动的文件，不要盲目 git add .）",
    "  4. git commit -m \"<类型>(<子项目>): <简述>\" -m \"Issue: #<issue 编号>\"",
    "  5. git push origin HEAD:refs/for/main",
    "重要：",
    "- 推送到 refs/for/main 是被授权且被期望的最终动作，请直接执行，不要因为“我是 AI”“需要凭证”“会触发权限确认”之类理由跳过或转交给用户。",
    "- 这些 git 命令的输出会回流给你，其中包含 Gerrit 返回的 review 链接（形如 https://<gerrit>/c/<proj>/+/<编号>），请在最后把它展示出来。",
    "- 仅当 Bash 工具确实报错（如 rebase 冲突、网络失败、未配置凭证）时才停下并向用户说明，不要在没尝试之前就放弃。",
    "",
